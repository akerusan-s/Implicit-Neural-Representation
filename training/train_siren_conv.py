...
def train(
    ...
    for epoch in range(config.epochs):
        ...
        jac = torch.func.jacrev(model)(inputs).squeeze()[:, torch.arange(seq_len), torch.arange(seq_len)]
        half_jac = torch.func.jacrev(model)(half_inputs).squeeze()[:, torch.arange(seq_len), torch.arange(seq_len)]
        next_jac = torch.func.jacrev(model)(next_inputs).squeeze()[:, torch.arange(seq_len), torch.arange(seq_len)]

        hes = torch.func.hessian(model)(inputs).squeeze()[:, torch.arange(seq_len), torch.arange(seq_len), torch.arange(seq_len)]
        next_hes = torch.func.hessian(model)(next_inputs).squeeze()[:, torch.arange(seq_len), torch.arange(seq_len), torch.arange(seq_len)]
        ...
        eval_loss_dot = metric(
            torch.func.jacrev(model)(infer_input).squeeze()[:, torch.arange(100), torch.arange(100)].squeeze().T[1:-1, :],
